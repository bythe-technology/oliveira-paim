...
export default function ContatoPage() {
  ...
  return <><PageHero className="company-hero" eyebrow="Contato" title="Vamos conversar sobre o próximo passo da sua empresa." text="Escolha o canal mais conveniente. Nossa equipe está pronta para entender o seu momento e orientar a melhor direção." image="/images/hero-contato-editorial.webp" imageAlt="Cena ilustrativa de ambiente para atendimento consultivo" /><section className="section"><div className="container contact-grid">{items.map(([Icon, label, value, href]) => <article key={label}><Icon size={34} weight="thin" /><span>{label}</span>{href ? <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer"><h2>{value}</h2></a> : <h2>{value}</h2>}</article>)}</div></section></>;
}
